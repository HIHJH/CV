Read me!
<GaussianSeparableColor.cpp>

-목적
: 이 코드는 RGB 이미지의 날카로운 에지가 무뎌지도록, 이미지가 더 부드러워지도록 하기 위해 R,G,B 채널 각각 Gaussian filtering을 수행하는 코드입니다. 이 코드에서는 gaussian filter를 Ws(s)와 Wt(t)로 나누어 2번 적용하여 연산을 비교적 빠르게 수행합니다.
...
-코드의 기본 설정
: 기본 코드는 "lena.jpg" 파일 이미지를 gaussian filtering 합니다. filter는 n = 1, sigmaT = 1, sigmaS = 1로 기본 설정되고 zero-paddle로 boundary process를 수행합니다. 그리고 원본 이미지는 "Original", filtering한 이미지는 "Gaussian Filter" 창에 띄워지도록 설정되어 있습니다. 그리고 코드 실행에 걸린 시간이 ms 단위로 print 됩니다.
