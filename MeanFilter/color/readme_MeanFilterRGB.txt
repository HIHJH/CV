...
-목적
: 이 코드는 RGB 이미지의 날카로운 에지가 무뎌지도록, 이미지가 더 부드러워지도록 하기 위해 R,G,B 채널 각각 mean filtering을 수행하는 코드입니다.
...
-코드의 기본 설정
: 기본 코드는 "lena.jpg" 파일 이미지를 mean filtering 합니다. filter는 n = 3으로 기본 설정되고 zero-paddle으로 boundary process를 수행합니다. 그리고 원본 이미지는 "Original", filtering한 이미지는 "Mean Filter" 창에 띄워지도록 설정되어 있습니다.
