...
-목적
: 이 코드는 opencv의 kmeans 함수를 이용한 clustering을 수행하여 컬러 이미지 내에서 비슷한 픽셀 값을 가지는 것들을 분류하는 코드입니다.
...
-코드의 기본 설정
: 기본 코드는 "lena.jpg" 파일의 이미지를 k-means clustering 합니다. sample1은 r,g,b color 값을 기준으로 clustering을 수행하고, sample2는 r,g,b,x,y color+position 값을 기준으로 clustering을 수행합니다.
