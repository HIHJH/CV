...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # forward pass
    # X*W1+b1 계산하고 ReLU 함수 적용하여 첫번째 hidden layer 계산
    hidden_layer = np.maximum(0, np.dot(X, W1) + b1)
    # hidden_layer*W2+b2 계산하여 scores 얻음
    ...
    #
    dscores = probs.copy()
    # 각 train data point의 정답 클래스에 대한 예측값의 loss를 계산하기 위해
    # dscores 배열에서 정답 클래스에 해당하는 요소의 값 1 감소
    dscores[np.arange(num_train), y] -= 1
    # loss의 미분값 계산하기 위해 scale 조정
    dscores /= num_train
    # backward pass
  ...
  def predict(self, X):
    ...
    # X에 대한 예측값 계산
    scores = self.loss(X)
    # 각 입력 데이터에 대한 예측 클래스 결정 (= 최대 스코어의 index )
